...
object(self)
  ...
  method private add_widget layout widget = match layout with
    | Port.Fixed -> table#attach ~left:2 ~top ~expand:`X ~fill:`NONE widget
    | Port.Field -> table#attach ~left:2 ~top ~expand:`X ~fill:`X widget
    | Port.Boxed -> table#attach ~left:2 ~top ~expand:`BOTH widget

  method private add_trail trail = 
    let layout = Port.get_layout trail in
    let widget = Port.get_widget trail in
    match layout with
      | Port.Fixed -> table#attach ~left:3 ~top ~expand:`NONE widget
      | Port.Field -> table#attach ~left:3 ~top ~expand:`NONE widget
      | Port.Boxed -> table#attach ~left:3 ~top ~expand:`Y widget
	...
	begin
	  ...
	  let rec fill x y = function
		...
		let layout = if y then Port.Boxed else 
		  if x then Port.Field else Port.Fixed in
		self#add_widget layout hbox#coerce ;
		self#add_trail trail
	    | ctrl::others ->
